...
for sim in sim_subfolders:
    ...
    d_ben = [[[] for i in range(2+55+3)] for i in range(7)]
    d_mal = [[[] for i in range(2+55+3)] for i in range(7)]
    ...
    for file in queue:
        ...
        # HM_Ben files
            ...
            with open(file, 'r') as f:
                for i, line in enumerate(f):
                    line_values = line.split(',')
                    d_ben[i][2+fileindex].append(float(line_values[0]))
        ...
        # HM_Mal files
            ...
            with open(file, 'r') as f:
                for i, line in enumerate(f):
                    line_values = line.split(',')
                    d_mal[i][2+fileindex].append(float(line_values[0]))
    ...
    for i in range(7):
        ...
        # Label 0 if benign, 1 if malicious
        d_ben[i][2+55+1].append(0)
        d_mal[i][2+55+1].append(1)
        ...
        d_ben[i][2+55+2].append(newsim*7 + (i+1))
        d_mal[i][2+55+2].append(newsim*7 + (i+1))
